            ...
            if(Math.sqrt(Math.pow((paperList[p].x - scissorList[s].x), 2) + Math.pow((paperList[p].y - scissorList[s].y), 2)) < 
                (paperList[p].width + scissorList[s].width) / 2){
                    ...
                    if(main.zombieMode)
                        ...
                        scissorList.push(new classes.Scissors(paperList[p].ctx, paperList[p].x, paperList[p].y, paperList[p].width, paperList[p].height, paperList[p].moveSpeed, paperList[p].moveState, main.canvas))
        ...
        for(let p = 0; p < paperList.length; p++){
            if(Math.sqrt(Math.pow((rockList[r].x - paperList[p].x), 2) + Math.pow((rockList[r].y - paperList[p].y), 2)) < 
                (rockList[r].width + paperList[p].width) / 2){
                    if(main.zombieMode)
                        paperList.push(new classes.Paper(rockList[r].ctx, rockList[r].x, rockList[r].y, rockList[r].width, rockList[r].height, rockList[r].moveSpeed, rockList[r].moveState, main.canvas))
        ...
        for(let r = 0; r < rockList.length; r++){
            if(Math.sqrt(Math.pow((scissorList[s].x - rockList[r].x), 2) + Math.pow((scissorList[s].y - rockList[r].y), 2)) < 
                (scissorList[s].width + rockList[r].width) / 2){
                    if(main.zombieMode)
                        rockList.push(new classes.Rock(scissorList[s].ctx, scissorList[s].x, scissorList[s].y, scissorList[s].width, scissorList[s].height, scissorList[s].moveSpeed, scissorList[s].moveState, main.canvas))
...
export function ChangeFindDistance(newDistance){
    maxFindDistance = newDistance;
}